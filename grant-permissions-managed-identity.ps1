param(
    [Parameter(Mandatory = $true, HelpMessage = "Display name of the managed identity or Automation account system-assigned identity")]
    [string]$ManagedIdentityDisplayName,

    [Parameter(Mandatory = $true, HelpMessage = "Microsoft Graph application permissions declared by the selected script")]
...
# Grant only the permissions explicitly supplied from the script metadata.
$PermissionsToAssign = @($CustomPermissions | Sort-Object -Unique)
...
#------------------------------------------------------------------------------
<#
# Grant the exact permissions declared by a selected script:
...
.\grant-permissions-managed-identity.ps1 -ManagedIdentityDisplayName "MyAutomation" -CustomPermissions @("User.Read.All") -EnableVerboseLogging

# Skip module check (if modules are already installed):
.\grant-permissions-managed-identity.ps1 -ManagedIdentityDisplayName "MyAutomation" -CustomPermissions @("User.Read.All") -SkipModuleCheck
#>
