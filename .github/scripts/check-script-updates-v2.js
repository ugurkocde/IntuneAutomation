...
// Generate slug from title (matching the website's logic)
...
// Check if a script is a remediation script
function isRemediationScript(scriptPath) {
  return scriptPath.includes('scripts/remediation/');
}

// Get the remediation folder name from path
function getRemediationFolderName(scriptPath) {
  const pathParts = scriptPath.split(path.sep);
  const remediationIndex = pathParts.indexOf('remediation');
  if (remediationIndex >= 0 && remediationIndex < pathParts.length - 2) {
    return pathParts[remediationIndex + 1];
  }
  return null;
}

// Group remediation scripts by folder
function groupRemediationScripts(scripts) {
  const remediationGroups = new Map();
  const standaloneScripts = [];

  for (const script of scripts) {
    if (isRemediationScript(script.path)) {
      const folderName = getRemediationFolderName(script.path);
      if (folderName) {
        if (!remediationGroups.has(folderName)) {
          remediationGroups.set(folderName, {
            detection: null,
            remediation: null,
            folder: folderName
          });
        ...
        const group = remediationGroups.get(folderName);
        if (script.path.includes('detect-')) {
          group.detection = script;
        } else if (script.path.includes('remediate-')) {
          group.remediation = script;
        }
      }
    } else {
      standaloneScripts.push(script);
    }
  }

  // Create combined remediation entries
  const combinedScripts = [];
  for (const [folderName, group] of remediationGroups) {
    if (group.detection && group.remediation) {
      // Create a combined title by removing "Detection" from the detection script title
      const combinedTitle = group.detection.name.replace(' Detection', '');
      const combinedSlug = generateSlug(combinedTitle);
      ...
      if (group.detection) standaloneScripts.push(group.detection);
      if (group.remediation) standaloneScripts.push(group.remediation);
    }
  }

  return [...standaloneScripts, ...combinedScripts];
}
...
async function checkAndNotify() {
  try {
    ...
    const updates = [];
    const newScripts = [];
    const remediationUpdates = new Map(); // Track remediation script updates
    ...
    // Group remediation scripts for proper display
    const groupedNewScripts = groupRemediationScripts(newScripts);
    const groupedUpdates = groupRemediationScripts(updates);
    
    // Write to GitHub Actions summary
    await writeToGitHubSummary(groupedNewScripts, groupedUpdates);
    
    // Send notifications if there are updates or new scripts
    if (groupedNewScripts.length > 0 || groupedUpdates.length > 0) {
      await sendNotifications(groupedUpdates, groupedNewScripts);
...
async function sendNotifications(updates, newScripts) {
  ...
  if (newScripts.length > 0) {
    ...
    for (const script of newScripts) {
      // Generate URL based on script type
      let url;
      if (script.isRemediation) {
        // Use the combined slug for remediation scripts
        url = `https://intuneautomation.com/script/${script.slug}`;
      } else {
        // Generate script ID from path (remove file extension)
        const scriptId = path.basename(script.path, path.extname(script.path));
        url = `https://intuneautomation.com/script/${scriptId}`;
      }
      ...
      emailHtml += `
                  <div style="background-color: #fafafa; padding: 24px; margin-bottom: 16px; border-radius: 2px; border: 1px solid #edebe9;">
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 16px;">
                      ...
                      ${script.isRemediation ? `
                      <tr>
                        <td style="padding: 8px 0;">
                          <span style="color: #605e5c; font-size: 14px;">Type:</span>
                          <span style="background-color: #e6f4ea; padding: 4px 12px; border-radius: 12px; color: #188038; font-size: 12px; margin-left: 8px; font-weight: 600;">Remediation Script (Detection + Remediation)</span>
                        </td>
                      </tr>` : ''}
                    </table>
                    <table width="100%" cellpadding="0" cellspacing="0">
