 ...
 *
 * The hosted MCP server (web/src/app/mcp) fetches this index from GitHub raw
 * at runtime and bundles the committed copy as a fallback, so it must be
 * regenerated and committed whenever scripts change (wired into CI).
 *
...
async function buildEntry(filePath, azureTemplates) {
  ...
  return {
    ...
    azureDeploy: azureTemplates[id]
      ? {
          deployUrl: azureTemplates[id].deployUrl,
          templateUrl: azureTemplates[id].templateUrl,
          supportsExistingAutomationAccount:
            azureTemplates[id].supportsExistingAutomationAccount === true,
        }
      : null,
    output: meta.output ?? "",
...
/** Per-script Azure Automation deployment links, keyed by script id. */
async function loadAzureTemplates() {
  try {
    const raw = await readFile(
      join(repoRoot, "azure-deployment-templates.json"),
      "utf8",
    );
    return JSON.parse(raw).templates ?? {};
  } catch {
    // The templates file is generated by a separate CI job; a missing or
    // malformed copy must not block index generation.
    return {};
  }
}
...
async function main() {
  ...
  const azureTemplates = await loadAzureTemplates();
  const scripts = [];
  for (const f of files) {
    scripts.push(await buildEntry(f, azureTemplates));
  }
